...
*Note that this is our first example of measuring a property of a part of something. Above, an instance of Uberon eye is part of an instance of Homo sapiens.  Other ontologies may have relevant parthood relations - for example Uberon says "`eye` `part of` some `visual system`" but it stops short of establishing a parthood chain between `eye` and `mammalia` for example.  In the future such standardizing axioms may be introduced which client ontologies can take advantage of.  Regardless, it is usually ok to establish parthood directly in one's aboutness instances.*
